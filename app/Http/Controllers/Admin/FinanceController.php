...
class FinanceController extends Controller
{
     ...
     */
    /**
     * Rebuild the bank balance from the account opening balance.
     *
     * The Opening Balance is stored as a transaction for audit/display, but
     * it is NOT added again during replay. The formula is always:
     *
     * Opening Balance + Credits - Debits = Running Balance
     *
     * Example:
     * 28,202 + 15,300 = 43,502
     * 43,502 - 43,000 = 502
     * 502 - 2,000 = -1,498
     *
     * This method is the single source of truth for balance_after and
     * current_balance, so old/wrong stored balances are repaired automatically.
     */
    private function recalculateBalanceChain(BankAccount $account): void
    {
        // Always start from the real opening balance stored on the account.
        $balance = (float) $account->opening_balance;

        // Replay every transaction chronologically. The opening transaction
        // itself is skipped because its amount is already in $balance.
        ...
        foreach ($transactions as $txn) {
            if ($this->isOpeningBalanceTransaction($txn)) {
                // Keep the opening row's displayed balance correct.
                $txn->forceFill([
            ...
            if ($txn->direction === 'credit') {
                $balance += $amount;
            } elseif ($txn->direction === 'debit') {
                $balance -= $amount;
            }

            // This is the balance AFTER this exact transaction.
            $txn->forceFill([
                'balance_after' => $balance,
            ])->saveQuietly();
        }

        // The account balance must equal the final running balance.
        $account->forceFill([
